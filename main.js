...
const { autoUpdater } = require('electron-updater'); // checa/baixa releases do GitHub (configurado em package.json > build.publish)
...
// ===== Memoria: 2 paineis = 2 processos de renderer (cada webview tem sua propria
// partition/sessao, entao o Chromium exige processo separado por conta; nao da pra
// mesclar). =====

// GPU ligada: o jogo usa WebGL de verdade. Desativar a GPU forca o Chromium a
// renderizar WebGL via software (SwiftShader), que e mais pesado pra CPU e mais
// instavel (perde contexto, engasga, corrompe textura) — pior ainda com 2 paineis
// simultaneos. Com driver de GPU presente (amdgpu/Mesa no Linux, ANGLE/Direct3D no
// Windows) a aceleracao e estavel; sem driver o Chromium cai pra software sozinho.
...
function logErro(origem, detalhe) {
  try {
    ...
    if (!errCabecalho) {
      errCabecalho = true;
      txt += `\n=== sessao de ${new Date().toLocaleString('pt-BR')} · PokeDreamGrid v${app.getVersion()} · Electron ${process.versions.electron} · ${process.platform} ${require('os').release()} ===\n`;
    }
...
ipcMain.handle('errlog:open', () => {
  try {
    if (!fs.existsSync(errFile())) fs.writeFileSync(errFile(), 'Nenhum erro registrado ate agora.\n');
    shell.showItemInFolder(errFile());
  } catch {}
});

// Atualizacao automatica: consulta as Releases do GitHub (repo configurado em
// package.json > build.publish), baixa sozinho e instala quando o usuario reiniciar
// ou fechar o app. So funciona em build empacotado (nao em `npm start`).
autoUpdater.autoDownload = true;
autoUpdater.autoInstallOnAppQuit = true;
autoUpdater.logger = { info: (m) => logErro('update', m), warn: (m) => logErro('update', m), error: (m) => logErro('update', m) };
...
const GAME = 'https://pokedream.com.br';
...
// UA consistente: remove o token "Electron/..." e congela a versão do Chrome em .0.0.0,
// casando com os client hints (navigator.userAgentData). Deriva da versão real do Chromium,
// então acompanha upgrades do Electron sozinho.
app.userAgentFallback = app.userAgentFallback
  .replace(/ Electron\/[\d.]+/, '')
  .replace(/(Chrome\/\d+)[\d.]+/, '$1.0.0.0');

// Notificacao do SO (alertas de queda de conta).
...
ipcMain.handle('webhook:send', (_e, url, text) => {
  try {
    ...
    // allowed_mentions vazio: nome de conta vem da configuracao do usuario, nao pode virar ping de @everyone
...
app.whenReady().then(() => {
  app.setAppUserModelId('br.com.pokedream.pokedreamgrid'); // notificacoes do Windows aparecem com o nome certo
  ...
  // Atualizacao automatica: checa as Releases do GitHub, baixa sozinho e so instala quando
  // o usuario aceitar reiniciar (ou ao fechar o app normalmente). Manual=true e quando o
  // usuario pediu explicitamente (menu/tray) e por isso merece feedback mesmo sem novidade.
  async function checarAtualizacao(manual) {
    if (!app.isPackaged) {
      if (manual) dialog.showMessageBox(win, { type: 'info', title: 'PokeDreamGrid', message: 'Checagem de atualização só funciona no app empacotado (build), não em "npm start".' });
      return;
    }
    try {
      const r = await autoUpdater.checkForUpdates();
      if (manual && r && r.updateInfo && r.updateInfo.version === app.getVersion()) {
        dialog.showMessageBox(win, { type: 'info', title: 'PokeDreamGrid', message: `Você já está na versão mais recente (${app.getVersion()}).` });
      }
    } catch (e) {
      logErro('update', (e && e.stack) || e);
      if (manual) dialog.showMessageBox(win, { type: 'error', title: 'Erro ao checar atualização', message: String((e && e.message) || e) });
    }
  }
  autoUpdater.on('update-available', (info) => {
    try { if (Notification.isSupported()) new Notification({ title: 'PokeDreamGrid', body: `Baixando atualização ${info.version}…` }).show(); } catch {}
  });
  autoUpdater.on('update-downloaded', async (info) => {
    const { response } = await dialog.showMessageBox(win, {
      type: 'info', title: 'Atualização pronta',
      message: `PokeDreamGrid ${info.version} baixado. Reiniciar agora para instalar?`,
      buttons: ['Reiniciar agora', 'Depois'], defaultId: 0, cancelId: 1
    });
    if (response === 0) autoUpdater.quitAndInstall();
  });
  ipcMain.handle('update:check', () => checarAtualizacao(true));
  setTimeout(() => checarAtualizacao(false), 10000); // checagem silenciosa pouco depois de abrir
  setInterval(() => checarAtualizacao(false), 6 * 60 * 60 * 1000); // e a cada 6h (app fica aberto o dia todo farmando)

  // Atalhos (funcionam mesmo com o jogo focado): Ctrl+1..2 expande painel, Ctrl+M mudo.
  Menu.setApplicationMenu(Menu.buildFromTemplate([{
    ...
    submenu: [
      ...
      { type: 'separator' },
      { label: 'Verificar atualizações', click: () => checarAtualizacao(true) }
    ]
  }]));
